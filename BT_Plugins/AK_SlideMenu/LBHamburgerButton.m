...
- (instancetype)initWithFrame:(CGRect)frame withHamburgerType:(LBHamburgerButtonType)type hamburgerState:(LBHamburgerButtonState)state lineWidth:(CGFloat)lineWidth lineHeight:(CGFloat)lineHeight lineSpacing:(CGFloat)lineSpacing lineCenter:(CGPoint)lineCenter color:(UIColor *)color {
...
- (void)switchOpen{
    if (_hamburgerState == LBHamburgerButtonStateHamburger) {
        [self animateButtonForward:TRUE];
...
- (void)switchClose{
    if (_hamburgerState == LBHamburgerButtonStateNotHamburger) {
        [self animateButtonForward:FALSE];
