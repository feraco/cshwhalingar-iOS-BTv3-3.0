...
#pragma mark -
#pragma mark - BTViewController Rotation

-(NSUInteger)supportedInterfaceOrientations {
    if (self.centerViewController) {
        if ([self.centerViewController isKindOfClass:[BT_navController class]]) {
            return [((BT_navController *)self.centerViewController).topViewController supportedInterfaceOrientations];
        }
...
#pragma mark -
#pragma mark - BTViewController Containment

- (void)setLeftMenuViewController:(BT_viewController *)leftSideMenuViewController {
...
- (void)setCenterViewController:(BT_viewController *)centerViewController {
    [self removeCenterGestureRecognizers];
    [self removeChildViewControllerFromContainer:_centerViewController];
    self.shadow = nil;
    
    CGPoint origin = ((BT_viewController *)_centerViewController).view.frame.origin;
    _centerViewController = centerViewController;
    if(!_centerViewController) return;
    
    [self addChildViewController:_centerViewController];
    [self.view addSubview:[_centerViewController view]];
    [((BT_viewController *)_centerViewController) view].frame = (CGRect){.origin = origin, .size=centerViewController.view.frame.size};
...
- (void)setRightMenuViewController:(BT_viewController *)rightSideMenuViewController {
...
- (void)removeChildViewControllerFromContainer:(BT_viewController *)childViewController {
...
- (void)setUserInteractionStateForCenterViewController {
    ...
    if([self.centerViewController respondsToSelector:@selector(viewControllers)]) {
        NSArray *viewControllers = [(BT_navController*)self.centerViewController viewControllers];
        for(BT_viewController* viewController in viewControllers) {
